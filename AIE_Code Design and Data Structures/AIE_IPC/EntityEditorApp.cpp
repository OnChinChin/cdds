...
{
	m_fileHandle = CreateFileMapping(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sizeof(Entity), m_sharedMem);
...
void EntityEditorApp::Update(float deltaTime)
{
	...
	//update all entities mapped into data

	//aka loop through each item data = entity
	for (int i = 0; i < ENTITY_COUNT; i++)
	{
